class PointLog < ActiveRecord::Base
  attr_accessible :point, :type, :user_id, :your_id
	...
	def self.link_in_without_fb user, your
		...
		edit_point(user, your.id, code, bd.point)
	...
	def self.independance user
		code = 3
		ret, limit_point, limit_node_cnt = PointLog.is_function_possible?(user, nil, code)[0]
		if ret
			edit_point(user, nil, code, limit_point)
		else
			return false
		end
	end
	...
	def self.betray user, your
		code = 4
		ret, limit_point, limit_node_cnt = PointLog.is_function_possible?(user, your, code)[0]
		if ret
			edit_point(user, your.id, code, limit_point)
		else
			return false
		end
	...
	def self.is_function_possible? user, your, code
		...
		when 4
			...
			limit_point = bd.point * user.node_cnt
			limit_node_cnt = bd.node_cnt
		when 5
			if (user.point + PointLog.deduct_point(code, your.node_cnt)) > 0 and user.node_cnt > bd.node_cnt)
				ret = true
	...
	def deduct_point(code, node_cnt)
		ret_point = 0
